...
default_cfgs = {
    ...
    'ccmlp_b3': _cfg(
        url='',
        input_size=(3, 300, 300), crop_pct=0.904),
    'ccmlp_b4': _cfg(
        url='',
        input_size=(3, 384, 384), crop_pct=0.922)
}

class Tokenizer(nn.Module):
    def __init__(self, img_size=224, use_conv_blocks=False, embed_dim=64):
        super().__init__()
...
class PatchMergingV2(nn.Module):
    ...
    def __init__(self, in_dim, out_dim):
        ...
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.reduction1 = nn.Linear(in_dim*4, in_dim*2, bias=False)
        self.act = nn.GELU()
        self.reduction2 = nn.Linear(in_dim*2, out_dim, bias=False)
        self.norm = nn.LayerNorm(in_dim*4)
...
class BasicStage(nn.Module):
    def __init__(self, num_blocks, seq_len, embedding_dim=[192,384], mlp_ratio=1, dropout_rate=0.1,
        ...
        if downsample_type == 'conv_resmlp':
            self.resMLP = True
        else:
            self.resMLP = False
        ...
        if self.downsample:
            ...
            elif downsample_type == 'mergev2':
                self.downsample_mlp = PatchMergingV2(embedding_dim[0], embedding_dim[1])
            else:
...
@register_model
def ccmlp_s_v3(pretrained=False, **kwargs):
    model_kwargs = dict(
...
@register_model
def ccmlp_s_v3_pe(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=3, num_blocks=[2,4,2], downsample_rate=8, mlp_ratio=[2,2,2], embedding_dim=[128,256,512],
        use_conv_blocks=True, positional_embedding='sinusoidal', **kwargs)
    return _create_ccmlp('ccmlp_s', pretrained=pretrained, **model_kwargs)

@register_model
def ccmlp_s_v3_convdown(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=3, num_blocks=[2,4,2], downsample_rate=8, mlp_ratio=[2,2,2], embedding_dim=[128,256,512], 
        use_conv_blocks=True, downsample_type='conv', **kwargs)
    return _create_ccmlp('ccmlp_s', pretrained=pretrained, **model_kwargs)

@register_model
def ccmlp_s_v3_shuffle(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=3, num_blocks=[2,4,2], downsample_rate=8, mlp_ratio=[2,2,2], embedding_dim=[128,256,512], 
        use_conv_blocks=True, use_mlp_shuffle=True, **kwargs)
    return _create_ccmlp('ccmlp_s', pretrained=pretrained, **model_kwargs)

@register_model
def ccmlp_s_v3_depthconv(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=3, num_blocks=[2,4,2], downsample_rate=8, mlp_ratio=[2,2,2], embedding_dim=[128,256,512], 
        use_conv_blocks=True, use_depthconv=True, **kwargs)
    return _create_ccmlp('ccmlp_s', pretrained=pretrained, **model_kwargs)
    
@register_model
def ccmlp_s_v3_convdown_depthconv(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=3, num_blocks=[2,4,2], downsample_rate=8, mlp_ratio=[2,2,2], embedding_dim=[128,256,512], 
        use_conv_blocks=True, downsample_type='conv', use_depthconv=True, **kwargs)
    return _create_ccmlp('ccmlp_s', pretrained=pretrained, **model_kwargs)

@register_model
def ccmlp_s_v3_resMLP(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=3, num_blocks=[2,4,2], downsample_rate=8, mlp_ratio=[2,2,2], embedding_dim=[128,256,512], 
        use_conv_blocks=True, downsample_type='conv_resmlp', use_depthconv=True, **kwargs)
    return _create_ccmlp('ccmlp_s', pretrained=pretrained, **model_kwargs)

@register_model
def ccmlp_s_v3_baseline(pretrained=False, **kwargs):
    model_kwargs = dict(
...
@register_model
def ccmlp_b0(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=3, num_blocks=[2,4,2], downsample_rate=8, mlp_ratio=[2,2,2], embedding_dim=[128,256,512], 
        use_conv_blocks=True, downsample_type='conv', use_depthconv=True, **kwargs)
    return _create_ccmlp('ccmlp_b0', pretrained=pretrained, **model_kwargs)

@register_model
def ccmlp_b0_v2(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=3, num_blocks=[2,4,2], downsample_rate=8, mlp_ratio=[2,2,2], embedding_dim=[128,192,256], 
        use_conv_blocks=True, downsample_type='conv', use_depthconv=True, **kwargs)
    return _create_ccmlp('ccmlp_b0', pretrained=pretrained, **model_kwargs)

@register_model
def ccmlp_b0_v3(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=3, num_blocks=[2,4,2], downsample_rate=8, mlp_ratio=[2,3,4], embedding_dim=[128,192,256], 
        use_conv_blocks=True, downsample_type='conv', use_depthconv=True, **kwargs)
    return _create_ccmlp('ccmlp_b0', pretrained=pretrained, **model_kwargs)

@register_model
def ccmlp_b0_2stage(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=2, num_blocks=[2,4], downsample_rate=8, mlp_ratio=[2,2], embedding_dim=[128,256,512], 
        use_conv_blocks=True, downsample_type='merge', use_depthconv=True, **kwargs)
    return _create_ccmlp('ccmlp_b0', pretrained=pretrained, **model_kwargs)

@register_model
def ccmlp_b1(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=3, num_blocks=[3,6,3], downsample_rate=8, mlp_ratio=[3,3,3], embedding_dim=[128,256,512], 
        use_conv_blocks=True, downsample_type='conv', use_depthconv=True, **kwargs)
...
def ccmlp_b2(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=4, num_blocks=[2,3,6,3], downsample_rate=4, mlp_ratio=[3,3,3,3], embedding_dim=[64,128,256,512], 
        downsample_type='conv', use_depthconv=True, **kwargs)
    return _create_ccmlp('ccmlp_b2', pretrained=pretrained, **model_kwargs)

@register_model
def ccmlp_b3(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=4, num_blocks=[3,4,12,4], downsample_rate=4, mlp_ratio=[2,2,2,2], embedding_dim=[128,256,512,1024], 
        downsample_type='conv', use_depthconv=True, **kwargs)
    return _create_ccmlp('ccmlp_b3', pretrained=pretrained, **model_kwargs)
...
def ccmlp_b4(pretrained=False, **kwargs):
    model_kwargs = dict(
        num_stage=4, num_blocks=[3,4,12,4], downsample_rate=4, mlp_ratio=[2,2,2,2], embedding_dim=[128,256,512,1024], 
        downsample_type='conv', use_depthconv=True, **kwargs)
    return _create_ccmlp('ccmlp_b4', pretrained=pretrained, **model_kwargs)